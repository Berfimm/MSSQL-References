...
---------------------------------------------------------------------
SELECT c.custid ,contactname, o.orderid,o.orderdate 
from Sales.Customers c
left join Sales.Orders o on o.custid = c.custid
where o.orderdate >= '2008-04-01'
order by  o.orderdate desc, custid;
---------------------------------------------------------------------
-- Task 2
-- 
...
-- Task 3
-- 
-- Copy the existing T-SQL statement from task 2 
--and modify it so that the result will return all employees 
--and be ordered by the manager’s first name. 
--Try first to use the source column name, 
--and then try to use the alias column name.
--
-- Execute the written statement and 
--compare the results that you got with the recommended result 
